...
사용자 요청 #81: iOS 홈 하단을 개인화 추천과 다음 단계 추천의 두 영역으로 확장한다. 각 영역은 중복 없는 공식 덱 3개이며 iPhone은 가로 스크롤, iPad는 3열 한 줄로 표시한다. 관련 단위·양 폼팩터 UI 회귀 후 PR 리뷰·CI를 거쳐 main 반영한다. Android 동등 확장은 이번 iOS 작업과 분리한다.

사용자 요청 #73 / PR #74: 관심사 뒤 4단계 레벨 확인과 첫 홈 추천→실제 학습 후 이어하기를 제공한다. 2026-08-29 사용자 진행 승인으로 로컬 5언어 후보 `codex/local-es-de-fr`를 통합했으며 원본 clone은 보존했다. 돌아가기 없는 2열 예시 카드와 양 플랫폼 관련 회귀·5언어 UI 검증을 마쳤다. 비활성 CI 복구·리뷰 뒤 main에 반영하며, 스토어·카탈로그 게시 gate는 별도다. 상세 검증은 `PROJECT_STATUS.md`와 최신 SHA 증빙, 반복 체크리스트는 `docs/LANGUAGE_EXPANSION_CHECKLIST.md`를 따른다.
