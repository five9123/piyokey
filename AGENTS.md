...
에이전트 실행 위치는 작업 목적으로 구분한다(2026-09-20 정민 승인, hankey와 공통).

1. 구현·통합·테스트처럼 오래 걸리고 진행을 지켜볼 가치가 있는 작업은 Orca
   worktree에서 실행한다. 실제 실행 로그가 해당 worktree 터미널과 카드에 남아
   Orca 화면에서 진행을 확인할 수 있어야 한다.
2. 조율·문서·Linear/ASC 등 CLI 작업과 짧은 검증은 Buzz 실행 환경을 그대로 쓴다.
3. 실행 환경과 무관하게 push된 PR, `release/evidence/<검증 SHA>.json` 증빙, Orca
   카드 상태 갱신을 함께 남긴다. 이 세 가지가 있으면 실행 환경이 달라도 추적이
   끊기지 않는다.
4. Orca에서 실행할 때도 Linear Dispatcher 자동 경유(Codex 소모)가 아니라 해당
   worktree 터미널에서 직접 실행을 기본값으로 한다. Codex 등 한정 자원 소모는
   사전 승인 대상이다.

## 활성 플랫폼 경계

iOS/iPadOS만 현재 제품 출시 범위다. Issue #175의 Mac Catalyst target은 macOS 14+
로컬 데모 범위이며 배포·스토어·클라우드 동기화 범위가 아니다. Mac 데모는 공식
콘텐츠, 커리큘럼, 6개 게임과 로컬 기록만 제공하고 사용자 덱·Deck Maker·피요컵을
노출하지 않는다. `android/`는 과거 포트의 참고 자료로 동결하며 일반 기능, 수정,
의존성 갱신, CI, 출시 gate에 포함하지 않는다. Android를
...
3. merge와 evidence commit까지 포함한 현재 PR head를 Claude가 검토해
   ...
   ancestor이고 evidence-only commit이 `release/evidence/*.json`을 추가·삭제·교체한
   것 외에 다른 파일을 바꾸지 않았는지 확인한다. 리뷰 뒤 head가 바뀌면 새 head를
   다시 검토한다.
