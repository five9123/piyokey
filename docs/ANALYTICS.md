...
The first privacy notice is versioned. On iOS, complete onboarding, the three hatch missions, and the app tour, request notification permission once, then show the notice once before Home. The notice has no toggles or provider terminology: **Participate and continue** enables both analytics and diagnostics, while **Continue without sharing** disables both. Refusing does not limit lessons, games, local decks, or purchased features. Settings keeps the two independent toggles, the privacy-policy link, and a review action that reuses the same two-button notice. Keep `PrivacyNoticePolicy.currentVersion` at 1 for this copy and interaction change so existing reviewed choices remain intact; increment it only when a later material collection or provider change requires renewed review.
