...
- iPad Pro 13-inch (M5): `d3917ea`의 주요 6화면 세로/가로·연습 회전/진행 보존·접근성 큰 글자 UI 3개 통과. 이후 변경은 이 검사에 포함되지 않은 퀴즈 가로 배치뿐이다.
...
증빙: `release/evidence/05834f7d5f558325195fed723670883a2110cb87.json`. 이 기록의 종합 결과는 열린 strict 출시 gate 때문에 `fail`이며, 개별 소스/선택 UI/이미지 검증 통과와 구분한다. 이후 현황·증빙 커밋은 앱 소스를 변경하지 않는다.
