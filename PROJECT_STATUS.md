...
## 2026-09-21 Mac 재개 후속 — iPad 입력 확인 완료

- 기존 TYP-123 / PR #176 후보 `25539b4de87738821c10f124e8a6da8fe83d5a9a`의 iPad 입력 확인 네 항목을 정민이 직접 통과했다. iPad Pro 11-inch 3세대 / iPadOS 26.6.2(23G90)에서 한글 입력, 앱 복귀 후 이어 입력, 화면 키보드 닫힘 유지, 초성 다시 하기 후 첫 입력에 문제가 없었다. 범위를 정한 수동 확인이며 전체 iPad 수락·성능 계측을 뜻하지 않는다.
- 결과는 [TYP-123 댓글](https://linear.app/typee/issue/TYP-123#comment-6dd33860)에 기록·재조회했다. 이슈는 In Review이며 DRI·후속 준비는 니모다.
- main `1f3027a2a32f2ac15fb814ba41f9decdec3074c2`의 실행 환경 문서를 기존 Mac branch에 통합한다. 저장소 이름 변경으로 오동작하던 workspace doctor의 원격 기준도 `five9123/piyokey`로 맞춘다. 이 후속 변경은 앱 제품 소스·리소스를 변경하지 않는다.
- 같은 후보의 signed Mac Debug build·실제 학습 화면을 다시 확인했다. 사람의 물리 한글 입력·삭제·발음/효과음, 창·숨김/복귀·저장·6개 게임 확인과 최종 HEAD 독립 리뷰·maintainer/보호 규칙 gate는 남는다. 기존 #187/#190 한정 보호 예외는 #176에 적용하지 않는다.

### 설정 버튼 표시 일관성 후속

- 9/21 정민이 오른쪽 위 설정 버튼의 위치·모양 변화를 보고했다. 홈 계열의 원형 커스텀 배경과 연습·초성의 맨 아이콘이 서로 달라, `PIYOKEY_MAC_DEMO`에서 홈 설정도 동일한 톱니 아이콘으로 맞춘다. iOS/iPadOS의 기존 버튼 렌더 분기는 유지한다.
- 학습 진입·종료 후 설정이 본문 헤더에서 타이틀바 탭 옆으로 옮겨지고 창 제목도 바뀌는 현상을 재현했다. Mac root 설정이 있는 NavigationStack의 실제 UINavigationBar에 공개 API `preferredBehavioralStyle = .pad`를 적용해 메뉴를 본문에 유지한다. 비교 앱에서 최초 표시·학습 왕복·탭 전환 후 위치가 유지되는 것을 확인했다. 버튼 배경의 상태별 변화는 위치 재배치와 분리해 남기며 사용자 수락을 대신하지 않는다. DRI·구현·검증 취합 니모, 새 HEAD 독립 검토 스펀지밥.
