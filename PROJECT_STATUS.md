...
| CI·병합 | GitHub Actions 활성. private 저장소의 현재 요금제에서는 branch protection/ruleset 사용 불가 | 모든 표시 PR check 성공 후에만 squash merge하는 수동 fail-closed gate 유지 |

## 열린 작업

| Issue | Project 상태 | 다음 한 단계 |
|---|---|---|
| #99 현행 문서 정리 | In Progress | 오래된 상태·온보딩 문서를 현재 원본 체계로 교체하고 docs PR CI 확인 |
...
1. #99 현행 문서 정리 완료.
2. Actions를 Node 24 호환 버전·`setup-java@v5`·commit SHA pinning으로 갱신하고 Dependabot·허용 Actions 범위를 점검.
3. 최신 `main`에서 iOS 1.1 새 RC를 만들고 #77 외부 gate와 TestFlight 실기기 QA 진행.
4. docs-only CI 분리와 정기 iOS unit·Android instrumentation workflow를 별도 최적화.
5. #7·#58·#19의 실기기 gate 처리. #8은 Later 유지.
