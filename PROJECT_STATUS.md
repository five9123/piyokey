...
| iOS 1.1 | `main` `18d1521`에 TYP-88 OS 10키 받침 경계 수정과 TYP-89 Settings 정보구조·segmented 배열 선택까지 병합됨. TestFlight 1.1 (12)는 Complete·설치 상태로 TYP-81 smoke에 사용 중이며 재아카이브·재업로드·재설치하지 않음 | TYP-90의 exact-head review·수동 병합 뒤 별도 build 13에서 iPhone·iPad 키 가이드 정책과 OS 10키 corpus를 검증. build 12 실측과 post-fix 실기기·release gate는 OPEN |
...
|---|---|---|
| TYP-90 iPad 전용 다음 키 가이드 | In Review / `codex/90-ipad-key-guide-policy` | iPhone·iPad focused simulator에서 Settings·덱 연습·커리큘럼 레슨·흐름·산성비를 검증하고 exact-head Claude review를 받는다. 병합 뒤 build 13 실제 iPhone·iPad·TestFlight gate는 OPEN이며 그 전 Done 처리하지 않음 |
...
1. TYP-90의 iPhone·iPad focused simulator evidence와 exact-head Claude review·수동 병합을 완료하되 build 13 실기기·TestFlight gate를 닫지 않는다.
2. 별도 build 13에서 TYP-90 키 가이드와 TYP-88 OS 10키 corpus, TYP-83의 iPhone·iPad OS 천지인 practice/lesson/Flow/Dictation 대표 복합모음과 두벌식 실제 오타 회귀를 확인한다.
3. clean `origin/main`에서 정확한 iOS 1.1 RC를 archive·TestFlight 처리하고 TYP-73 연속 단어 전환, TYP-77 입력원 경고와 row-13 최종 QA를 함께 수행한다.
