...
최근 통합 기준선: PR #86, `77a7dd64b23f62d95862da0c2b945dfc2d90a55e` — 연습 챕터 5·6 스테이지 확장까지 반영
...
| iOS 1.1 | `main` `77a7dd6`; 업로드된 `1.1 (7)`은 최신 소스 이전 | build 7 TestFlight 처리·1.1 연결 완료, App Review 미제출 | Issue #87 포함 최종 선택 기능을 통합한 새 build 번호 RC, 현지어·미디어·IAP·Paid Apps·Sandbox·Files/iCloud/AirDrop·1,000항목 gate |
...
|---|---|---|---|
| #87 / PR #88 `.typedeck` BCP 47·deck schema v2 | Verify | `codex/87-typedeck-bcp47` / `/private/tmp/piyokey-issue87-typedeck` | 웹 writer golden 호환은 통과; Actions 활성화 승인과 웹 변경 커밋·PR 연결 뒤 ready 전환 |
...
- Android retention 단위 테스트와 앱 Kotlin 컴파일, iOS 커리큘럼·번들 음원·5언어/한국어 UI 제외 테스트, Python 언어 범위·preflight 24개, 저장소 preflight와 50개 목표 교차 검증을 통과했다. 최신 `main` 통합 뒤 같은 관련 범위를 재검증했다. 기기 수동 QA·스토어 제출·배포는 하지 않았다.
...
| 열린 PR | #88 draft; 로컬 다중 플랫폼 회귀와 웹 writer golden 호환 통과 | Actions 활성화 승인과 웹 변경 커밋·PR 연결 뒤 ready 전환 |
