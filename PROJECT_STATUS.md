...
| iOS 1.1 | `main` `43d65b3`에 TYP-73 OS IME 세션 리셋이 병합됨. TYP-77 PR #147은 marked ASCII 방어 보강이며 실기기 `IMETextField` 천지인 원인은 아직 미확정 | TYP-77 row 13 콜백의 committed/marked U+ 코드포인트를 iPhone에서 기록해 원인과 수정 적합성을 확정한 뒤에만 build 11 archive 진행 |
...
|---|---|---|
| TYP-77 OS 천지인 ASCII guard 오판 | In Review / PR #147 / `codex/77-cheonjiin-ascii-guard` | PR은 marked ASCII를 확정 입력원 경고에서 제외하는 defensive hardening으로 유지. iPhone row 13 코드포인트·committed/marked 관측, Practice 배너 0회, 5개 직접 입력 게임×5단어, 일본어 로마자 IME 체감, 정확한 후속 TestFlight build의 iPhone·iPad 증빙은 `gate:device`로 유지 |
...
1. TYP-77 row 13을 iPhone `IMETextField`에서 실행해 OS 천지인 `ㄹㆍㅣㅣ → 레` 전 콜백의 U+ 코드포인트·committed/marked 상태를 기록하고 PR #147의 적합성을 확정한다.
2. 원인과 수정이 확정되고 focused review·merge가 완료된 뒤 clean `origin/main`에서 정확한 iOS 1.1 build 11을 archive·TestFlight 처리한다.
