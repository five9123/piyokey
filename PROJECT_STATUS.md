...
| iOS 1.1 | `main` `e9684bf`에 TYP-43 build 19 분석 배선·UI 정정까지 병합됐다. build 19는 TestFlight 분석 검증에만 사용하고 심사 제출하지 않으며, TYP-101 플릭 미리보기와 TYP-102 온보딩 정정을 포함한 다음 제출 후보는 build 20이다 | TYP-101·TYP-102 source gate와 exact-head review 뒤 최신 clean `origin/main`으로 build 20을 만든다. 동의 ON/OFF 네트워크, Crashlytics dSYM, iPhone·iPad 회귀와 기존 계정·권리·IAP·store gate 전에는 출시 완료로 표현하지 않음 |
...
| TYP-43 iOS/iPadOS 1.1 출시 | In Progress / source main `e9684bf` | build 19에서 동의 ON/OFF 네트워크와 Crashlytics dSYM만 검증한다. TYP-101·TYP-102 병합 뒤 clean `origin/main` build 20에서 실기기·계정·권리·IAP·store gate를 순서대로 닫음 |
...
1. build 19에서 동의 ON 이벤트 수신·동의 OFF 무전송·Crashlytics 테스트 크래시와 dSYM을 확인하되 심사 제출에는 사용하지 않는다.
2. TYP-101 플릭 방향 미리보기와 TYP-102 온보딩 정정의 focused source gate·exact-head review·maintainer 승인을 각각 완료한다.
3. 두 변경 병합 뒤 최신 clean `origin/main`에서 PostHog token과 Firebase plist를 주입한 build 20 archive를 만든다.
4. build 20에서 TYP-101·102와 TYP-85·94·95·97·98 및 기존 TYP-73·77·82·83·88·93 회귀를 iPhone·iPad 실기기로 확인한다.
