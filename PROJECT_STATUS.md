...
| iOS 1.1 | TestFlight `1.1 (10)`에서 재현한 TYP-73 OS 한국어 키보드 조합 잔존 회귀의 소스 수정은 main `43d65b3`에 병합됨. TYP-75 병합 전에는 build 11을 archive하지 않음 | TYP-75 review·merge 뒤 정확한 `origin/main`으로 build 11을 만들고 iPhone·iPad × 두벌식·천지인 실기기 gate 검증 |
...
|---|---|---|
| TYP-75 Settings·MyPage 정리 | In Review / PR #146 / Linear Project `iOS/iPadOS 1.1 Global Release` (`99007f20-e78b-4ede-8c02-5fc47a062053`) | Claude 2차 findings와 main `43d65b3` 충돌을 수정하고 focused unit·iPhone SE portrait·iPad landscape 검증 완료. Claude 재검토·CI·merge 뒤 build 11 및 실기기·스토어 gate를 별도 진행 |
| TYP-73 iOS/iPadOS OS 한국어 키보드 단어 전환 조합 잔존 | Merged / PR #145 → main `43d65b3` / build 11 source | TYP-75 병합 뒤 최신 `origin/main`으로 build 11을 만들고 Practice+5개 게임, iPhone·iPad × 두벌식·천지인 × 연속 10단어를 확인하며 천지인 `대형`·`쇼파`를 포함 |
...
1. TYP-75 PR #146의 Claude 재검토·CI를 통과시키고 merge한다.
2. TYP-75 merge 뒤 clean `origin/main`에서 정확한 iOS 1.1 build 11을 archive·TestFlight 처리한다.
3. build 11에서 Practice+5개 직접 입력 게임의 iPhone·iPad × 두벌식·천지인 × 연속 10단어, 첫 자모·콤보·정확도·점수·first responder를 검증한다. 천지인 `대형`·`쇼파`도 포함하며 통과 전 TYP-43을 재개하지 않는다.
