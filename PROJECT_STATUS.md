...
| iOS 1.1 | `main` `0ed4777`에 TYP-82 OS IME 단어 전환, TYP-84 직접 입력 게임 chrome, TYP-86 일본어 콘텐츠 fallback, TYP-83 OS 천지인 committed 중간 상태 수정이 병합됨. TestFlight 1.1 (12)는 Complete·설치 상태로 TYP-81 smoke에 사용 중이며 재아카이브·재업로드·재설치하지 않음 | TYP-88 PR #154의 exact-head review·수동 병합 뒤 별도 build 13에서 iPhone·iPad OS 10키 corpus를 검증. build 12 snapshot·dangling 실측, iPad/post-fix 실기기, full HancoTests·release gate는 OPEN |
...
|---|---|---|
| TYP-88 OS 10키 받침 경계 중간 상태 | In Review / PR #154 / `codex/88-os-10key-batchim-boundary` | source-confirmed Class A/B와 negative matrix를 검증하고 exact-head review를 진행. Debug probe는 별도 예약 기기에서만 실행하며 build 12 snapshot·dangling 관찰과 정확한 build 13 iPhone·iPad corpus·TYP-81 로그는 OPEN |
...
1. TYP-83의 merged-tree focused 검증·evidence·Claude review를 완료하되 iPad와 TestFlight gate를 닫지 않는다.
2. 후속 TestFlight에서 TYP-83의 iPhone·iPad OS 천지인 practice/lesson/Flow/Dictation 대표 복합모음과 두벌식 실제 오타 회귀를 확인한다.
